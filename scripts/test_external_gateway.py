#!/usr/bin/env python3
"""
Enterprise AI Agent Gateway & Cloud CDN - External Verification & E2E Test Suite
==========================================================================
...
def run_tests(gateway_url: str = "http://127.0.0.1:8080"):
    ...
    try:
        r = client.get("/api/v1/notes/current?sample=sample_meet_85min")
        data = r.json()
    ...
    try:
        payload = {
            ...
            "speaker_mapping": {"[화자 1]": "홍길동 팀장", "[화자 2]": "이몽룡 담당"}
        }
        r = client.post("/api/v1/notes/replace-speakers", json=payload)
        updated = r.json().get("updated_markdown", "")
        assert_test("POST /api/v1/notes/replace-speakers", "홍길동 팀장과 이몽룡 담당가 논의했습니다." in updated, "(1-Click Batch Replacement verified)")
    except Exception as e:
    ...
    try:
        # Test CORS headers
        r = client.options("/api/v1/health", headers={"Origin": "https://portal.enterprise.co.kr", "Access-Control-Request-Method": "GET"})
        cors_hdr = r.headers.get("access-control-allow-origin")
        assert_test("CORS Headers Injection", cors_hdr in ["*", "https://portal.enterprise.co.kr"] or r.status_code == 200, f"(Access-Control-Allow-Origin: {cors_hdr})")
    except Exception as e:
